...
4.  **Tools**: Is there a Skill/Tool for this? -> **"The Tool Shed" Rule**.
    - *Rule*: Check `.agent/skills/` and Cursor's available skills before planning.
5.  **Accounts**: Are you logged into the correct CLI account? -> **"The Identity Check" Rule (CRITICAL)**.
    - *Rule*: Before ANY `vercel`, `convex`, or `gh` commands, verify you are logged into: **`workwithharshdesai@gmail.com`**.
    - *Action*: Run `vercel whoami`, `npx convex login status`, and `gh auth status` to confirm identity.
...
- **Action**:
    1.  **Research**: 
        ...
        - **Review Previous Chats**: Look for similar queries or past failures in Cursor chat history to learn and improve execution.
...
4. **Strategic Pause**: When stuck, pause and audit the whole situation/codebase. Think strategically and tactically to uncover blind spots. Figuring out solutions requires catching your blind spots.
...
- **`/d` command**: Cursor slash command at `.cursor/commands/d.md` — enforces the 7-step agentic workflow. Type `/d` in chat + your task.
- **`/v` command**: Cursor slash command at `.cursor/commands/v.md` — Tesla Pre-Visualization Protocol. Type `/v` in chat for deep pre-visualization before execution.
